...
def main(args):
    ...
    stats_file_epoch = open(args.exp_dir / "stats_epoch.txt", "a", buffering=1)
    # print(" ".join(sys.argv))
    print(" ".join(sys.argv), file=stats_file_epoch)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, ((x, y), _) in (pbar := tqdm(enumerate(loader, start=epoch * len(loader)), total=len(loader), desc=f"Epoch {epoch:02d}")):
            ...
            pbar.set_postfix(loss=loss_epoch/(seen_samples), lr=lr)
            seen_samples += 1
        ...
        # print(json.dumps(stats))
        print(json.dumps(stats_epoch), file=stats_file_epoch)
